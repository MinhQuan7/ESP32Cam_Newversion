...
//_______________Function Detele File_______________________________
void deleteVideo() {
  Serial.println("Access to deleteVideo Function ");
  
  // Initialize an array to store the 5 oldest file names and their timestamps
  double del_numbers[5] = {999999999, 999999999, 999999999, 999999999, 999999999};
  char del_numbernames[5][50] = {{0}, {0}, {0}, {0}, {0}};
  ...
  while (file) {
    if (!file.isDirectory()) {
      ...
      // Filter only numeric and '.' characters from the file name to get the timestamp
      for (int x = 0; x < 50; x++) {
        if ((foldname[x] >= '0' && foldname[x] <= '9') || foldname[x] == '.') {
          // Keep digits and dots
        } else if (foldname[x] != 0) {
          foldname[x] = ' ';
        }
      }

      double file_number = atof(foldname);
      if (file_number > 0) {
        // Insert this file into the array of 5 oldest files if it qualifies
        for (int i = 0; i < 5; i++) {
          if (file_number < del_numbers[i]) {
            // Shift older entries down and insert the new one
            for (int j = 4; j > i; j--) {
              del_numbers[j] = del_numbers[j-1];
              strcpy(del_numbernames[j], del_numbernames[j-1]);
            }
            del_numbers[i] = file_number;
            strcpy(del_numbernames[i], file.name());
            break;
          }
        }
      }
  ...
  // Now delete the 5 oldest files
  for (int i = 0; i < 5; i++) {
    if (del_numbers[i] < 999999999) {
      Serial.printf("Deleting file: %s, number: %f\n", del_numbernames[i], del_numbers[i]);
      deleteFolderOrFile(del_numbernames[i]);
    }
  }

  // Calculate disk usage after deletion
  float full = 1.0 * SD_MMC.usedBytes() / SD_MMC.totalBytes();
  Serial.printf("Disk is %.1f%% full ... \n", 100.0 * full);
...
void loop() {
  ...
  if (IncludeInternet > 1) {
    if (read13 == 2 && !InternetOff) {
      Serial.println("Shutting off wifi ...");
      logfile.println("Shutting off wifi ...");
      filemgr.end();
      stopCameraServer();
      //WiFiManager wm;
      //wm.disconnect();
    ...
    if (read13 == 0 && InternetOff) {
      Serial.println("Starting the wifi ...");
      logfile.println("Starting the wifi ...");
      init_wifi();
      Serial.println("Starting Web Services ...");
      startCameraServer();
      start_Stream_81_server();
